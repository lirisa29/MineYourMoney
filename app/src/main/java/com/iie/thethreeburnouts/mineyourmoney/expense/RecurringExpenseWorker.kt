...
class RecurringExpenseWorker (
    ...
    override fun doWork(): Result = runBlocking { //(App Dev Insights, 2024)
        ...
        Log.d(
            TAG,
            "Received inputData -> walletId=$walletId | amount=$amount | recurrence=$recurrence | userId=$userId"
        )
        ...
        if (newExpenseId != -1L) {
            ...
            Log.w(
                TAG,
                "Failed to add recurring expense (insufficient funds or DB error). Retrying 🔁"
            )
